...
class syntax_plugin_twitter extends DokuWiki_Syntax_Plugin {
	...
	private function signRequest($endpointUrl, $params = array()) {
		...
		$signature = base64_encode(hash_hmac('sha1', ('GET&' . $this->urlencode($endpointUrl) . '&' . $this->urlencode($sign_base_string)), $this->_oauth_consumer_secret . '&' . ($this->_oauth_token_secret != null ? $this->_oauth_token_secret : ''), true));
